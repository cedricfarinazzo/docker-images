...
GitHub Actions for the monorepo. One release workflow at the root; two workflows per image (build on tag + nightly rebuild). All jobs are structured as `setup → build → merge → aliases` so per-arch legs run on native runners and are stitched into multi-arch manifests by digest.
