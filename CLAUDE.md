...
- `actions-runner/` — GitHub Actions self-hosted runner. Extends `ghcr.io/actions/actions-runner` (Ubuntu base), pinned to a specific version via `versions.json`'s `runner_version` (also mirrored as the `ARG RUNNER_VERSION` default in the Dockerfile, which Renovate bumps; `versions.json` is the source of truth for release/nightly builds and is bumped by hand), with Docker CLI + Compose v2 + Buildx, Node.js 22 + yarn, yq (multi-arch via `TARGETARCH`), and a full build toolchain (`build-essential`, `cmake`, `python3`, `shellcheck`, etc.). No version matrix axes beyond the base runner pin — single variant, two arches (amd64 + arm64). Tags: `latest`, `<semver>`, `<major>.<minor>`, `<major>`, `nightly`, `nightly-YYYYMMDD`.
